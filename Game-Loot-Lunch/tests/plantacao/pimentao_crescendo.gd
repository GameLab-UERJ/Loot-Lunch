...
const cena_pimentao = preload("res://tests/plantacao/pimentao.tscn") 
...
func _tentar_dropar_pimentoes() -> void:
	var quantidade_atual_no_chao = _contar_pimentoes_perto()
	if quantidade_atual_no_chao < max_itens_no_chao:
		...
		for i in range(quantidade_a_dropar):
			dropar_novo_pimentao()


func _contar_pimentoes_perto() -> int:
	var contador = 0
	var todos_os_itens = get_tree().get_nodes_in_group("itens_dropados") 
	
	for item in todos_os_itens: 
		if item is Item and item.item_name == "Pimentão":
			if global_position.distance_to(item.global_position) < raio_busca: 
				contador += 1
	return contador


func dropar_novo_pimentao() -> void:
	var novo_drop = cena_pimentao.instantiate() 
